...
| Optical imagery | The thesis describes Landsat 8 as the optical reference. [USGS permits Landsat reuse and redistribution with source acknowledgement](https://www.usgs.gov/faqs/are-there-any-restrictions-use-or-redistribution-landsat-data), but the surviving 30 m GeoTIFFs have generic filenames and no product identifier or download metadata, so the exact USGS scene cannot be verified. | Raw source rasters are not included. Selected thesis map figures are shown in the README. |
| Google Earth Pro imagery | Used for visual inspection, 3D terrain review, and manual checking of drainage divides. Google's [Geo Guidelines](https://about.google/brand-resource-center/products-and-services/geo-guidelines/) require visible attribution. | A selected attributed project figure is shown in the README. Editable KMZ/KML files and raw imagery are not included. |
| DEM | The surviving 30 m raster contains coordinate and elevation information but no provider, product ID, or licence record. | The raw raster is not included. A derived thesis figure is shown in the README. |
...
| Assessment polygons and final classes | The polygons reflect the author's interpretation and thesis workflow; the classes are the author's model output. | Selected thesis figures and the final result table are included. |
